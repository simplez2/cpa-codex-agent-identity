...
## 同步保护：发布前调查与 v0.3.18 上线

以下是 v0.3.15 回档阶段的调查。**这些修复现已随 v0.3.18 的正式插件和
镜像上线**，完整资产及代理验收见 [v0.3.18 发布记录](releases/v0.3.18.md)。
...
- v0.3.18 已让 CPA 文件设置优先于导入默认值，保留显式 `false`。
...
修复没有回填或覆盖 v0.3.15；它属于有独立校验值的 v0.3.18。旧版本的
同步器不会因新版本发布而自动获得这些增强。此页不能据此宣称 Agent
Identity JWT/AgentAssertion 或所有未来 CPA 功能已经获得完整原生兼容。
