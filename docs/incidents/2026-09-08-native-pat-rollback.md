...
> 本页保留回档当时的记录。后续修复已随 v0.3.18 正式资产上线，见
> [v0.3.18 发布验收](../releases/v0.3.18.md)；下文的“尚未发布”是历史阶段，
> 不代表当前版本仍未部署。
...
回档前的三个代理 TCP 建连均在 3 秒探测窗口内超时，回档脚本没有清空、
绕过或替换它们。随后原生页面出现三次成功的代理字段 PATCH，代理被清空
并落盘；没有用备份覆盖这些后续编辑。

沿用更新后的当前配置，再测试**线上默认路由**：luna HTTP 200，文本、
finish、[DONE] 齐全；首字 1.58 秒、完整流 1.81 秒，成功计数正常增长。
这证明当前无代理路由恢复，不代表原先的坏代理变得可用。真实可用代理的
端到端矩阵、AgentAssertion 的原生控件/代理支持仍需单独完成，不能由上述
无代理成功外推。
