...
当前源码、pluginVersion、根目录 VERSION、已发布 registry 和可直接安装资产均为 v0.3.10。后续版本仍必须等 Linux 资产、GitHub Release、checksums 和下载验证全部完成后，才能在单独的发布提交中切换 registry。
...
当前插件不再依赖外挂卡片按钮。确认安装的是包含 ResourceRoute 的 v0.3.10 插件（源码与已发布 registry 均为 v0.3.10），CPA 的 `plugins.enabled` 和该插件配置的 `enabled` 都为 `true`，然后重启 CPA。CPA 资源入口是 `/v0/resource/plugins/codex-agent-identity/open`，正常应返回 HTML wrapper；若仍为 404，通常是插件没有注册成功、CPA 使用不支持资源路由的旧版本，或 CPAMC/CPA 仍在使用旧插件进程。直接入口 `/agent-identity/` 仍可作为回退。
