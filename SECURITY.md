...
- Treat the CPA auth directory as plaintext secret storage: managed files contain the upstream credential in `access_token` plus a revocable `sidecar_client_key` for compatibility/JWT routing. Native PAT model execution uses the upstream token directly, not that bridge key. Restrict ownership, permissions, backups, and exports accordingly.
