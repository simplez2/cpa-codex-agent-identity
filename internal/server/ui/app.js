...
const unavailableMessageType = 'cpa-codex-agent-identity:unavailable';
const managementBridgeEnabled = embeddedInCPAMC && window.parent !== window && isValidBridgeNonce(bridgeNonce);
const pluginUIBodyPrefix = ")]}',\n";
const pluginManagementAPIURL = resolvePluginManagementAPIURL();
...
function resolvePluginManagementAPIURL() {
  const marker = '/v0/resource/plugins/codex-agent-identity/';
  const path = window.location.pathname;
  const index = path.toLowerCase().lastIndexOf(marker);
  if (index < 0 || path.slice(index + marker.length).replace(/\/+$/, '').toLowerCase() !== 'ui') return '';
  return path.slice(0, index) + '/v0/management/codex-agent-identity/ui-api';
}

function shouldFallbackFromPluginBridge(error) {
  return Boolean(pluginManagementAPIURL && error && (error.status === 404 || error.status >= 500));
}

function notifyPluginBridgeUnavailable(status) {
  if (!managementBridgeEnabled || window.parent === window || pluginFallbackRequested) return;
  pluginFallbackRequested = true;
  window.parent.postMessage({
    type: unavailableMessageType,
    nonce: bridgeNonce,
...
async function api(path, options) {
  ...
  let response;
  if (pluginManagementAPIURL) {
    const forwardedHeaders = {};
    Object.keys(requestOptions.headers || {}).forEach(function (name) {
      if (name.toLowerCase() === 'content-type') forwardedHeaders['Content-Type'] = requestOptions.headers[name];
    });
    response = await fetch(pluginManagementAPIURL, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        body: requestOptions.body === undefined ? '' : String(requestOptions.body)
      })
    });
  } else {
    ...
    response = await fetch('./api/' + path, Object.assign({}, requestOptions, { headers: headers }));
  }
  let payload = {};
  let responseText = '';
  try { responseText = await response.text(); } catch (_) { responseText = ''; }
  if (pluginManagementAPIURL && responseText.indexOf(pluginUIBodyPrefix) === 0) {
    responseText = responseText.slice(pluginUIBodyPrefix.length);
  }
  try { payload = responseText ? JSON.parse(responseText) : {}; } catch (_) { payload = {}; }
  if (!response.ok) {
    const error = new Error(response.status === 401
      ? '管理密码不正确'
      : (payload.error || responseText || '请求失败 (' + response.status + ')'));
...
async function refresh(options) {
  ...
  try {
    ...
    renderSummary({});
    identitiesBox.replaceChildren();
    if (shouldFallbackFromPluginBridge(error)) {
      setStatus('CPA 无法连接 sidecar，正在尝试备用管理入口…', 'error');
      notifyPluginBridgeUnavailable(error.status);
    } else if (refreshOptions.fromBridge && error.status === 401) {
